...
python3 - "$frames" "$expected_frames" <<'PY'
import sys
...
if missing:
    raise SystemExit(
        f"missing {len(missing)} frames; first gaps: "
        + ", ".join(str(frame) for frame in missing[:12])
    )
print(f"validated {expected} contiguous map cinematic frames")
PY

duration=$(ffprobe -v error -show_entries format=duration -of default=nw=1:nk=1 "$soundtrack")
image_rate=$(python3 - "$expected_frames" "$duration" <<'PY'
import sys
