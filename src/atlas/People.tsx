...
export function People({model}:{model:PlaceModel}){
  const renderedModel=useRef<PlaceModel|null>(null);
  ...
  useFrame(()=>{
    if(!boxes.current||!heads.current||!limbs.current)return;if(worldClock.delta===0&&renderedModel.current===model)return;renderedModel.current=model;let bi=0,hi=0,li=0;const storm=model.id==='urithiru'?0:model.id==='shinovar'?worldClock.storm*.2:worldClock.storm;
    ...
    for(const [personIndex,p] of people.entries()){
      ...
      const visible=1,iriali=model.id==='kasitor'||model.id==='rall-elorim',skin=iriali?'#bda062':p.skin,hair=iriali?'#dbc17d':personIndex%5===0?'#746b61':'#352f29';
