...
function ribbon(b:ModelBuilder,path:XY[],width:number,height:number){
  const curve=new T.CatmullRomCurve3(path.map(p=>new T.Vector3(p[0],0,p[1])));const points=curve.getPoints(35);const positions:number[]=[];const colors:number[]=[];
  for(let i=0;i<points.length-1;i++){
    ...
    for(let band=0;band<14;band++){
      ...
      const color=new T.Color(['#ae8c76','#c6b8a0','#b99379','#ded0b4'][band%4]);
      for(const side of [-1,1]){
        ...
        positions.push(...a,...e,...d,...d,...e,...f);for(let v=0;v<6;v++)colors.push(color.r,color.g,color.b);
      }
    }
  }
  ...
  const list=b.buckets.get('stone')??[];list.push(g);b.buckets.set('stone',list);
}
export function buildReferenceKholinar():PlaceModel{
  ...
  for(let x=-275;x<290;x+=13)for(let z=-250;z<255;z+=13){
    ...
    if((pz< -95&&pz> -170&&Math.abs(px)<100)||Math.hypot(px,pz)<24)continue;
  ...
  const market=map([640,899]);for(let j=-5;j<=5;j++)b.stall(market[0]+j*8,.2,market[1]+6,j%2?'#885b50':'#58767c');
  ...
  return result(b,'kholinar',[515,410,550],[0,16,0],[market[0]+28,11,market[1]+32],[market[0],3,market[1]],640);
}
export function buildReferenceAzimir():PlaceModel{
  const b=new ModelBuilder(),rng=random(515);b.box([0,-6,0],[720,12,840],'#a69473');
...
export function buildReferenceThaylen():PlaceModel{
  const b=new ModelBuilder(),rng=random(331);
  ...
  for(let i=0;i<9;i++){
    ...
    if(i%2===0){b.sphere([x-29,-3.8,z+7],[5,2.5,13],palette.wood);b.box([x-29,11,z+7],[12,14,.2],'#cec6ac');b.cylinder([x-29,10,z+7],.2,22,palette.wood);}
  }
...
export function buildReferenceVedenar():PlaceModel{
  const b=new ModelBuilder(),rng=random(603);polygon(b,[[-6000,-10000],[6000,-10000],[6000,180],[550,205],[270,195],[110,230],[-80,208],[-310,175],[-6000,185]],0,60,'#87917b');
  ...
  return result(b,'vedenar',[510,360,510],[0,15,-50],[31,12,-4],[0,4,-35],600,{y:-43,size:2500,shallow:false});
}
