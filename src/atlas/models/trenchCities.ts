...
export function buildYeddaw():PlaceModel{
  const b=new ModelBuilder(),rng=random(615);
  ...
  for(let i=0;i<=steps;i++){
    ...
    if(i%2===0)for(const side of [-1,1]){
      ...
      const house=new ModelBuilder();house.building(0,y,0,8,6,10+rng()*7,['#c0aa83','#a87b5f','#bec0a5','#a7aa93'][i%4]);const g=house.finish('trench_dwelling');g.position.set(x,0,z);g.rotation.y=-tangent+(side<0?Math.PI:0);
      g.updateMatrixWorld(true);g.traverse(o=>{if(o instanceof T.Mesh){const geometry=o.geometry.clone();geometry.applyMatrix4(o.matrixWorld);const surface=o.name.endsWith('_window')?'window':'stone',bucket=b.buckets.get(surface)??[];bucket.push(geometry);b.buckets.set(surface,bucket);o.geometry.dispose();(o.material as T.Material).dispose();}});
    }
  ...
  const bulk=new T.ExtrudeGeometry(upper,{depth:42,bevelEnabled:false,curveSegments:96});bulk.rotateX(-Math.PI/2);b.add(bulk,'#a69b7e',[0,-4,0]);bulk.dispose();
  const fields=new T.ShapeGeometry(upper,96);fields.rotateX(-Math.PI/2);b.add(fields,'#83915e',[0,38.04,0]);fields.dispose();
  for(let i=0;i<steps;i++){
  ...
  const street=new T.BufferGeometry();street.setAttribute('position',new T.Float32BufferAttribute(floor,3));street.computeVertexNormals();b.add(street,'#b2a387');street.dispose();
...
export function buildSesemalex():PlaceModel{
  const b=new ModelBuilder(),rng=random(768);
  ...
  for(let cut=-3;cut<=3;cut++){
    ...
    for(const side of [-1,1]){
      ...
      for(let z=-length/2;z<length/2;z+=22){
        ...
        b.box([x+side*34,34,z],[31,.5,19],'#9ca17c');
      }
    }
