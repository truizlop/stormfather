...
import {hangingBell} from './landmarkDetails';
...
export function buildKharbranth():PlaceModel{
  const b=new ModelBuilder([0,0,110],'plaster'); const rng=random(17);
  ...
  for(let tier=0;tier<9;tier++){
    ...
    // Individual coping stones make the terrace edge read as construction.
    const copingCount=Math.ceil(half*2/1.4),copingWidth=half*2/copingCount;
    for(let k=0;k<copingCount;k++)b.box([-half+(k+.5)*copingWidth,y-.25,z+9],[copingWidth-.035,1,2],k%5?'#b8b8a3':'#a4ac9a',[0,0,0],'ashlar');
    ...
    if(tier%2===0){const x=half-13;b.box([x,y+14,z-4],[7,1,7],palette.cream);for(const dx of [-2.8,2.8])for(const dz of [-2.8,2.8])b.box([x+dx,y+9,z-4+dz],[.65,10,.65],palette.cream);hangingBell(b,[x,y+10,z-4]);b.cone([x,y+16,z-4],5,4);}
  }
